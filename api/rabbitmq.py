...
import pika
import sys, json
from helper import checkKeyExists
...
class RabbitMQ:
    ...
    def createConnection(self, queue_obj):
        try:
            ...
            port = queue_obj['port'] if checkKeyExists(queue_obj, 'port') else '5672'
            vhost = queue_obj['host'] if checkKeyExists(queue_obj, 'host') else '/'
            ...
            if heartbeat != None and blocked_connection_timeout != None:
                return pika.BlockingConnection(pika.ConnectionParameters(server, port, vhost, credentials, heartbeat=heartbeat, blocked_connection_timeout=blocked_connection_timeout))
            else:    
                return pika.BlockingConnection(pika.ConnectionParameters(server, port, vhost, credentials))
    ...
    def postQueue(self, queue_obj):
        try:
            ...
            channel.basic_publish(
                ...
                body=json.dumps(queue_obj),
                properties=pika.BasicProperties(
                    delivery_mode=pika.spec.PERSISTENT_DELIVERY_MODE
                ))
            channel.close()
            rabbit_mq_connection.close()
            return True, "Pushed data to queue successfully"
        except Exception as e:
